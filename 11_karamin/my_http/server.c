...
int perform_post(int sock, char** buffer, int* total_received, int* buf_memory_allocated)
{
	...
	if (content_length_pointer)
	{
		...
		if ((content_length = atoi(length)) == 0)//НЕ РАБОТАЕТ!!! Ошибка сегментации!!!
		{
			FILE *to_write = fdopen(sock, "w");
			throw_error(to_write, 403, title_403, "ничего не было выбрано(((", referrer, "post использован без файла.");
			fclose(to_write);
			return 403;
		}
	}
